...
export function createSegments(Cesium, viewer, segments) {
    ...
    for (var i = 0; i < segments.length; i++) {
        ...
        segments[i].forEach((position) => {
            positions.push(Cesium.Cartesian3.fromDegrees(position.lon, position.lat, position.alt * 1000));
        })
        // 创建任务线
    ...
    viewer.screenSpaceEventHandler.setInputAction(function onMouseClick(movement) {
        ...
        for (var i = 0; i < segments_entities.length; i++) {
            var segment = segments_entities[i];
